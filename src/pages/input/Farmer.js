...
const Farmer = () => {
  const { userId, userAddress, userFullName, setIsLoading } =
  ...
  const storeInputDataOnBlockchain = async () => {
    try {
      ...
      if (error.message.includes("Transaction has been reverted by the EVM")) {
        ...
        alert(
          "Transaction reverted. Please check your contract logic, gas limit, or other possible issues."
        );
      }
    }
  };
