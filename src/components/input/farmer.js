...
const Farmer = () => {
    ...
    const storeInputDataOnBlockchain = async () => {
        try {
            ...
            await contract.methods.addFarmerDetails(fullName, faddress,year, cropSeason, culArea, zone, seed, weightpadddy, farmselprice,totalCostString,costPerKgString).send({ from: accounts[0] });
    ...
    useEffect(() => {
        if (web3) {
            ...
            const contractInstance = new web3.eth.Contract(FarmerDetailsContract.abi, contractAddress);
